...
def cli_main(args=None):
    ...
    parser.add_argument("--transmit_dim", type=int, default=None)
    ...
    tensorboard_logger = TensorBoardLogger("lightning_logs", name=f'{args.enc_type}/{script_args.known_dataset}/')
    lr_logger = LearningRateMonitor(logging_interval='epoch')

    trainer = pl.Trainer.from_argparse_args(args, callbacks=[InitializeGaussians()], logger=tensorboard_logger)
    # trainer = pl.Trainer.from_argparse_args(args)
    trainer.test(model,
                 dm.train_dataloader())  # 这一次是计算高斯，并且is_tested=1,然后return给下一次用 # First iteration will calculate Gaussians, second will do the test.
