...
class GMM_VAE_Contrastive(pl.LightningModule):   
    ...
    def log_test_statistics(self,values,pred_labels,labels):
        ...
        df['Pos acc'] = pos_acc
        df['Neg acc'] = neg_acc
        df.to_csv('output/cifar10/version_{}/Threshold.csv'.format(self.trainer.logger._version))
    ...
    def plot_roc_curve(self,all_data,labels,all_sift_distances):
        ...
        df.to_csv('output/cifar10/version_{}/FPR_TPR.csv'.format(self.trainer.logger._version))

    def test_epoch_end(self, outputs):
        if not os.path.exists('output/cifar10/version_{}'.format(self.trainer.logger._version)):
            os.mkdir('output/cifar10/version_{}'.format(self.trainer.logger._version))
    ...
    def log_images(self,x,x_hat,label):
        ...
        self.trainer.logger.experiment.add_image(tag='recon_samples_{}'.format(label),img_tensor=img)
        img = x
        # img = Image.fromarray(np.squeeze(ndarr))
        # self.trainer.logger.experiment.log_image(
        #     'true_samples',
        ...
        self.trainer.logger.experiment.add_image(tag='true_samples_{}'.format(label), img_tensor=img)
