...
from models.resnet_encoders_semantic_temp import resnet18_encoder_VIB
...
class GMM_VAE_Contrastive(pl.LightningModule):
    def __init__(
        ...
        self.perturbation = 0.0014
        self.temperature = 500
        valid_encoders = {
            'resnet18_semantic_VIB_temp': {'enc': resnet18_encoder_VIB, 'dec': None},
        }
    ...
    def step(self, batch, batch_idx):
        ...
        output,[dec_mu,dec_logvar] = self.encoder(x,self.noise)
        kl = 0.01 * self.KLD(dec_mu,dec_logvar)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # inputs.grad.zero_()
        output,[dec_mu,dec_logvar] = self.encoder(inputs, self.noise)

        # No perturbation
        baseoutput = output.data/self.temperature
        baseoutput = baseoutput - torch.max(baseoutput,dim=1,keepdim=True)[0]
        basescore = torch.max(F.softmax(baseoutput),dim=1)[0].cpu().numpy()

        # ODIN
        maxIndexTemp = torch.argmax(output,dim=1)
        pred_labels = maxIndexTemp.cuda()
        ce_loss = x.shape[0] * self.criterion(output, pred_labels)
        ce_loss.backward()
        ...
        # perturbation
        var_inputs = torch.add(inputs.data,-self.perturbation,gradient).cuda()
        outputs,[dec_mu,dec_logvar] = self.encoder(var_inputs,self.noise)
        outputs = outputs/self.temperature
        outputs = outputs - torch.max(outputs,dim=1,keepdim=True)[0]
        odinscore = torch.max(F.softmax(outputs),dim=1)[0].detach().cpu().numpy()
        return basescore,odinscore,y
    def validation_step(self, batch, batch_idx):
        # z, y, x_hat, x, _, _, _ = self.test_step(batch, batch_idx)
        # return z, y, x_hat, x
        x, y = batch
        logit,[_a,_b] = self.encoder(x,self.noise)
    ...
    #     if self.log_tsne:
    #         print("T SNE start")
    #         x_te_proj_pca = TSNE(n_components=2, perplexity=30, n_iter=600, learning_rate=200).fit_transform(all_data)
    #         x_te_proj_df = pd.DataFrame(x_te_proj_pca[:, :2],
    #                                     columns=['Latent Space Projeciton 1', 'Latent Space Projection 2'])
    #         if self.unknown_dataset == 'lsun-resize':
    #             text_labels = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck',
    #                            'LSUN Resized']
    #         elif self.unknown_dataset == 'imagenet-resize':
    #             text_labels = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck',
    #                            'Imagenet Resized']
    #         else:
    #             raise ValueError("checkdataset")
    #         text_labels_array = np.take(text_labels, all_labels)
    #         x_te_proj_df['label'] = text_labels_array
    #         fig = plt.figure()
    #         x_te_proj_df.to_csv(f'{self.unknown_dataset}_JSCC.csv')
    #         ax = sns.scatterplot('Latent Space Projeciton 1', 'Latent Space Projection 2', data=x_te_proj_df,
    #                              palette='tab20',
    #                              hue='label',
    #                              linewidth=0,
    #                              alpha=0.6,
    ...
    #         fig.savefig(f'T-SNE_{self.unknown_dataset}_JSCC.eps',format='eps',dpi=300)
    #         # self.trainer.logger.experiment.add_figure('T-SNE With Unknown', fig)
    #         print("T SNE end")
    def test_epoch_end(self, outputs):
        torch.cuda.empty_cache()
        all_basescore = torch.vstack([torch.tensor(x[0]) for x in outputs]).cpu().numpy()
        all_odinscore = torch.vstack([torch.tensor(x[1]) for x in outputs]).cpu().numpy()
        values_list = [all_basescore,all_odinscore]
        method = ['base','odin']
        labels = torch.vstack([x[2] for x in outputs]).cpu().numpy()

        def fpr_tpr(values, labels, threshold):
            neg_samples_count = float(np.sum(labels == self.class_num))
            pos_samples_count = float(np.sum(labels < self.class_num))
            neg_vals = values[labels == self.class_num]
            pos_vals = values[labels < self.class_num]
            fp_count = float(np.sum(neg_vals >= threshold))  # 大于threshold,于是认为是positive，但实际上是negative(从neg_val里筛选出来的)
            tp_count = float(np.sum(pos_vals >= threshold))  # 大于threshold,于是认为是positve，而实际上也是postive
            return fp_count / neg_samples_count, tp_count / pos_samples_count
        ...
        for i,values in enumerate(values_list):
            ...
            tpr_at_095 = 0
            for t in tqdm(range(len(thresholds))):
                fp, tp = fpr_tpr(values, labels, thresholds[t])  # tp_cor
                if tp >= 0.95:
                    fpr_at_095 = fp
                    tpr_at_095 = tp
                fpr.append(fp)
                tpr.append(tp)
            self.log(f"tpr-fpr-095-{method[i]}", fpr_at_095)
            self.log(f"tpr-at-095-{method[i]}", tpr_at_095)
            self.log(f"tnr-at-tpr-095-{method[i]}", 1 - fpr_at_095)
            fig = plt.figure()
            plt.plot(fpr, tpr, color='darkorange')
            plt.plot([0, 1], [0, 1], color='navy', linestyle='--')
            plt.xlim([-0.00099, 1.0])
            ...
            plt.xlabel('False Positive Rate')
            plt.ylabel('True Positive Rate')
            plt.title(f'Roc Curve-{method[i]}')
            # self.trainer.logger.experiment.log_image('ROC', fig, description=f'ROC TEST')
            self.trainer.logger.experiment.add_figure(f'ROC-{method[i]}', fig)
            self.log(f'ROC-AUC-Score-{method[i]}', auc(fpr, tpr))
