...
class GMM_VAE_Contrastive(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        z = mu
        l_features = [torch.flatten(self.encoder.get_layer_output(x,self.noise,i),1) for i in range(1,5)]  + [mu]
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.log_tsne:
            ...
            x_te_proj_df = pd.DataFrame(x_te_proj_pca[:, :2], columns=['Latent Space Projeciton 1', 'Latent Space Projection 2'])
            ...
            x_te_proj_df.to_csv(f'{self.unknown_dataset}_propsoed.csv')
            fig = plt.figure()
            ...
            fig.savefig(f'T-SNE_{self.unknown_dataset}_proposed.eps', format='eps', dpi=300)
            print("end tsne")
    ...
    def initialize_gaussians(self):
        ...
        elif self.enc_type == 'resnet18_semantic_temp':
